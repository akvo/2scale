...
class ViewPeriodDetail extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        \DB::statement(
            "
            CREATE OR REPLACE VIEW rsr_detail_period_data AS
                SELECT
                    rr.order,
                    rp.id as project_id,
                    rp.title as project_title,
                    rr.id result_id,
                    rr.parent_result as parent_result_id,
                    ...
                    rpv.rsr_dimension_value_id as dimension_value_id,
                    rvtts.title as dimension_value_title,
                    rpv.value as dimension_actual_value
                    FROM rsr_results rr
                    ...
                    LEFT JOIN rsr_period_dimension_values rpv ON rpv.rsr_period_id = rps.id
                    LEFT JOIN rsr_dimension_values rdv ON rpv.rsr_dimension_value_id = rdv.id
                    LEFT JOIN rsr_dimensions rd ON rd.id = rdv.rsr_dimension_id
                    ...
                    LEFT JOIN rsr_titleables rvtt ON rvtt.rsr_titleable_id = rdv.id
